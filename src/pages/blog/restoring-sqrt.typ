#import "/lib.typ": page
#import "/common/blog/restoring-sqrt.typ": *
...
it with the next pair and subtract $chevron.l 0q_(k-1)01 chevron.r$ from it (this
notation is used to represent concatenation). The result will determine $q_(k-2)$,
in the same way as before. On the next step, we would group the result with the
next pair and subtract $chevron.l 00 q_(k-1)q_(k-2) 01 chevron.r$ from it, determining
$q_(k-3)$, and so on. This process is repeated until there are no more pairs.
The remainder of the last subtraction is $R$.
