...
$chevron.l 0q_(k-1)01 chevron.r$ (essa notação foi usada para representar concatenação).
O resultado irá determinar $chevron.l q_(k-2) chevron.r$. O próximo passo seria agrupar
novamente com o próximo par e subtrair $chevron.l 00 q_(k-1)q_(k-2) 01 chevron.r$,
encontrando $q_(k-3)$ etc. Esse processo é repetido até que não haja mais pares.
O resto da última subtração corresponde a $R$.
