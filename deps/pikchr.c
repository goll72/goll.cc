...
static const struct {
  ...
  { "Ok",                               -42 },  /* PATCH: Non-standard addition */
  { "OldLace",                     0xfdf5e6 },
  { "Olive",                       0x808000 },
  { "OliveDrab",                   0x6b8e23 },
  { "Orange",                      0xffa500 },
...
static void pik_append_clr(Pik *p,const char *z1,PNum v,const char *z2,int bg){
  ...
  // PATCH: implement "ok" color (CSS variable)
  if ( x==-42 ){
    snprintf(buf, sizeof(buf)-1, "%svar(--%s)%s", z1, bg ? "background" : "color", z2);
  }else{
    r = (x>>16) & 0xff;
    g = (x>>8) & 0xff;
    b = x & 0xff;
...
static void pik_append_style(Pik *p, PObj *pObj, int eFill){
  ...
  // PATCH: ok
  if( (pObj->fill>=0 || pObj->fill==-42) && eFill ){
    int fillIsBg = 1;
  ...
  if( pObj->sw>=0.0 && (pObj->color>=0.0 || pObj->color==-42) ){
    PNum sw = pObj->sw;
...
static void pik_append_txt(Pik *p, PObj *pObj, PBox *pBox){
  ...
  for(i=0; i<n; i++){
    ...
    if( pObj->color>=0.0 || pObj->color==-42 ){
